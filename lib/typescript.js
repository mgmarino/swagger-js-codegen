...
function convertType(swaggerType) {
    ...
    } else if (_.isArray(swaggerType.allOf)) {
        var derivesList = [];
        typespec.tsType = 'object';
        typespec.properties = [];
        _.forEach(swaggerType.allOf, function(embeddedType) {
            if (!_.isString(embeddedType.$ref)) {
              embeddedType.type = 'object';
            }
            var convertedType = convertType(embeddedType);
            if (convertedType.tsType === 'ref') {
                derivesList.push(convertedType.target);
            } else {
                typespec.properties = _.concat(typespec.properties, convertedType.properties);
            }
        });
        if (!_.isEmpty(derivesList)) {
            typespec.derives = {
                extendsList: derivesList.join(", ")
            };
        }
    ...
    typespec.isNotRequired = !!swaggerType.readOnly;
